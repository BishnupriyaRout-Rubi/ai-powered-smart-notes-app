...
function ViewNote() {
  ...
  // ✅ FIX: useCallback added (important for useEffect)
  ...
  useEffect(() => {
    ...
    loadFiles();
  }, [id, navigate, loadFiles]); // ✅ FIX added loadFiles
          ...
          {!Array.isArray(files) || files.length === 0 ? (
            ...
            files.map((file) => (
              <div key={file.id} style={{ marginTop: "10px" }}>
                <a
                  href={`${process.env.REACT_APP_API_URL}/${file.filePath}`} // ✅ FIX (localhost removed)
                  target="_blank"
                  rel="noreferrer"
                  style={{ color: "#ffd86b" }}
                >
